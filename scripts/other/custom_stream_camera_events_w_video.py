...
Saves:
    - Original camera stream → videos/original.mp4
    - Event camera stream → videos/event.mp4
    - Runs for 30 seconds automatically
"""
...
if __name__ == '__main__':
    ...
    try:
        ...
        #     controls={"FrameDurationLimits": (frame_us, frame_us)} 
            # af_mode=controls.AfModeEnum.Auto # added
        # )
        config = picam2_.create_video_configuration(
            main={"format": "BGR888", "size": (args.width, args.height)},
            controls={
        ...
        # CONTINUOUS AUTO FOCUS (NO LOCKING)
        try:
            picam2_.set_controls({
                "AfMode": controls.AfModeEnum.Auto
            })
            print("[INFO] Continuous autofocus enabled", flush=True)
        except Exception as e:
            print("[WARN] Autofocus not available:", e, flush=True)
        ...
        print("[INFO] Autofocus started", flush=True)
    ...
    try:
        while True:
            ...
            if time.time() - start_time > runtime:
                print("[INFO] runtime reached, exiting...", flush=True)
                break
            
            # trigger refocus every second - could try something with triggering on sharpness from Laplacian
            if frame_count % 25 == 0:  # ~1 seconds at 50 FPS
                picam2_.set_controls({
                    "AfTrigger": controls.AfTriggerEnum.Start
                })
            frame_count += 1
